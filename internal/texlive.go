...
func (c *Compiler) compileWithTexlive(content string, files []FileEntry, enqueuedAt time.Time, projectID string, cacheSession *CacheSession) *CompileResult {
	receivedAt := time.Now()
	...
	cacheEnabled := cacheSession != nil && projectID != "" && isMultiFile
	...
	if cacheEnabled {
		tempDir, fileChanges, isIncremental = cacheSession.PrepareIncrementalWorkspace(files, c.RequestID)
		if tempDir != "" {
			shouldCleanup = false
		}
	}
	...
	if pdfData, err := os.ReadFile(pdfPath); err == nil {
		...
		if cacheEnabled {
			cacheSession.StoreCompilation(files, tempDir, pdfData, sha256Hex, c.RequestID, "texlive")
		}
